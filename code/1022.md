...
> 1. 通过处理 **\<sstream\>** 库来处理字符串,
> 2. getline(istream, string, split_char)获取每一行的代码, 或者以某个字符作为分隔
> 3. 使用 **map**和**set**来存储对应映射, 方便直接查找
> 4. 一定注意最后输出id的时候有限制, 就是**7-bits digit**的数字, 所以在printf的时候, 输出格式要为 **"%07d"**